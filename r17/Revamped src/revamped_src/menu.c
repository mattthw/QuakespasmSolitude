...
void M_Main_Draw (void)
{
	...
	else
	{
		...
		M_Print (120, 0, va("Naievil's Custom: Version %4.2f", (float) KUR_VERSION));
}
...
//        M_DrawTransPic (72, 32, Draw_CachePic ("gfx/menu/multi_0.lmp") );
	    ...
	    if (m_multiplayer_cursor == 7)
	        M_PrintWhite ((0/2) - ((10*8)/2), 200,  "Remove Bot");
        else
	        M_Print ((0/2) - ((10*8)/2), 200,  "Remove Bot");

	    if (m_multiplayer_cursor == 8)
	        M_PrintWhite ((46) - ((26*8)/2), 216,  "Co-op Player Camera Change");
        else
	        M_Print ((46) - ((26*8)/2), 216,  "Co-op Player Camera Change");

	    if (serialAvailable || ipxAvailable || tcpipAvailable)
	    	return;
	    M_PrintWhite ((50) - ((27*8)/2), 232, "No Communications Available");
    }
	else
	{
	    M_DrawTransPic (16, 4, Draw_CachePic ("gfx/qplaque.lmp") );
...
{
	{"start", "Entrance"},		// 0
	{"Maze", "Small Test place"},
	{"Foundation", "Halo 2's best map"},
